...
process.stdin.on('data', data => {
  ...
  if (totalLength >= 8192) {
    ...
    totalLength = writeBuffer.byteLength % 4
    const writeCount = writeBuffer.byteLength - totalLength
    process.stdout.write(Buffer.from(writeBuffer.slice(0, writeCount).toString(), 'base64'))
...
process.stdin.on('end', () => {
  if (buffers.length > 0) {
    const writeBuffer = Buffer.concat(buffers)
    process.stdout.write(Buffer.from(writeBuffer.toString(), 'base64'))
  }
})
